                ...
                if (canvasUsers.Count > 0)
                {
                    foreach (User professor in canvasUsers)
                    {
                        ...
                        foreach (Section section in sections)
                        {
                            ...
                            HttpResponseMessage response = await httpClient.GetAsync($"{_configuration.GetConnectedString("CanvasRootUri")}/api/v1/courses/{sectionId}/users?per_page=1000&role_id={canvasStudentRoleId}");
